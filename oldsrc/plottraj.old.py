...
if __name__ == "__main__":
  ...
  state = read_raw_cm1_state(time, um=10, vm=0.0)
...
# Setting up a colormap that's a simple transtion
  ...
  plot = plot_wz(state['u'][klevel,:,:], state['v'][klevel,:,:], state['xe'], state['ye'], z = state['zc'][klevel], ax=ax, xlim=x_limits, ylim=y_limits)
  
  for n in N.arange(0,ntraj):
    xp = x[:nstep:10,n]/1000.
    yp = y[:nstep:10,n]/1000.
    b = (z[nstep-1,n]/1000.-cmin)/(cmax-cmin)
    g = 0.0
    r = 1-b
    ax.plot(xp, yp, color=(r,g,b), linewidth=1.)

  ax.set_xlim(x_limits[0], x_limits[1])
  ax.set_ylim(y_limits[0], y_limits[1])
  
  cb = P.colorbar(CS3) 
  cb.set_label("Height of Initial Parcel Location (km)")
...
#   zp = z[:nstep:10,:]/1000.
# 
#   ax.plot(xp, yp, zp, label='TrajData')
#   ax.legend()
# 
#   P.show()
#   
